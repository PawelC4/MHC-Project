...
/**
 * Each quest entry has:
 *   quest(station) → human-readable instruction shown to the player
 *   clipLabel      → short, visually concrete label used for CLIP verification
 *
 * Keep clipLabels under ~6 words and as visual as possible.
 * CLIP works best with noun-phrase descriptions of what is *in* the image,
 * not instructions like "photograph X" or "find Y".
 */
const QUEST_TEMPLATES = [
  {
    quest: station => `Photograph the station's name sign with you in the frame at ${station.name}.`,
    clipLabel: 'a subway station name sign',
  },
  {
    quest: () => `Find the station's tile art or mosaic and capture a close-up detail shot.`,
    clipLabel: 'colorful subway tile art or mosaic',
  },
  {
    quest: station => `Step outside and photograph the most interesting building visible from the ${station.name} exit.`,
    clipLabel: 'a building exterior on a city street',
  },
  {
    quest: () => `Find a street-level detail — a fire hydrant, manhole cover, or signpost — that feels uniquely NYC.`,
    clipLabel: 'a fire hydrant, manhole cover, or street sign',
  },
  {
    quest: station => `Locate the oldest-looking storefront near ${station.name} and document it.`,
    clipLabel: 'an old storefront or shop facade',
  },
  {
    quest: () => `Find something yellow near the station — a taxi, awning, or signage — and frame your shot around it.`,
    clipLabel: 'a yellow taxi cab or yellow awning',
  },
  {
    quest: station => `Capture the busiest corner near ${station.name} at peak energy.`,
    clipLabel: 'a busy city street intersection with people',
  },
  {
    quest: station => `Find a piece of street art, sticker art, or graffiti within one block of ${station.name}.`,
    clipLabel: 'street art graffiti or mural on a wall',
  },
  {
    quest: station => `Photograph the view looking directly up from the sidewalk near ${station.name}.`,
    clipLabel: 'looking up at tall buildings from the street',
  },
  {
    quest: () => `Find a local food cart or bodega and photograph something that represents the neighborhood.`,
    clipLabel: 'a street food cart or corner store bodega',
  },
];

/**
 * Return a random quest object { quest: string, clipLabel: string } for a given station.
 * @param {object} station
 * @returns {{ quest: string, clipLabel: string }}
 */
export function generateQuest(station) {
  const template = QUEST_TEMPLATES[Math.floor(Math.random() * QUEST_TEMPLATES.length)];
  return {
    quest: template.quest(station),
    clipLabel: template.clipLabel,
  };
}
...
export function generateAdventure(userLat, userLng, options = {}) {
  ...
  if (reachable.length === 0) {
    ...
    const questData = generateQuest(pick.station);
    return {
