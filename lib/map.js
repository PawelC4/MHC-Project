 ...
 */
export async function getDirections(originLat, originLng, destLat, destLng, destStationLines) {
  ...
  return _buildStubDirections(originLat, originLng, destLat, destLng, destStationLines);
...
function _parseGoogleDirectionsResponse(googleResponse) {
  ...
  const steps = leg.steps.map(step => {
    ...
    if (isTransit) {
      instruction = instruction.replace(/\b(train|subway)\b/gi, '🚇');
    }
 ...
 * @private
 */
function _buildStubDirections(oLat, oLng, dLat, dLng, destStationLines = []) {
  ...
  const lineInfo = destStationLines?.length > 0
    ? `(${destStationLines.join('·')})`
    : '';
  const subwayInstruction =
    `Board a ${boardDirection} 🚇` + (lineInfo ? ` ${lineInfo}` : '');
...
export function initLeafletMap(containerId, userLat, userLng, destLat, destLng, destName) {
  ...
  _routeLayer = window.L.polyline(
    [[userLat, userLng], [destLat, destLng]],
    {
      ...
      dashArray: '8, 12',
      opacity: 0.8,
      className: 'leaflet-route-animating',
    }
  ).addTo(_leafletMap);

  // ── Subway line color map ─────────────────────────────────
  // Every ROUTE code from the actual subway_lines.geojson, mapped to MTA official colors.
  // Division 1 = IRT, Division 2 = IND, Division 3 = BMT
  const ROUTE_COLORS = {
    // ── IRT Division 1 ───────────────────────────────────────
    // Broadway/7th Ave Line → 1·2·3 (red)
    '1':  '#EE352E', '3':  '#EE352E',
    // Eastern Parkway Line → 2·3·4·5 (red/green shared trunk)
    '8':  '#EE352E',  // Eastern Pkwy (2/3 side)
    // White Plains Rd Line → 2·5
    '6':  '#EE352E', '9':  '#EE352E',
    // New Lots Line → 3
    '11': '#EE352E',
    // Clark Street Line → 2·3
    '7':  '#EE352E',
    // Lexington Ave Line → 4·5·6 (green)
    '13': '#00933C', '14': '#00933C',
    // Eastern Parkway Line → 4·5 (green side)
    '34': '#00933C',
    // Jerome Ave Line → 4
    '12': '#00933C',
    // Lenox Ave Line → 2·3 (express) / also serves 4·5
    '10': '#00933C',
    // Dyre Ave Line → 5
    '16': '#00933C',
    // Pelham Line → 6
    '17': '#00933C',
    // Flushing Line → 7 (purple)
    '18': '#B933AD',
    // 42nd St Shuttle → S (gray)
    '69': '#808183',
 
    // ── IND Division 2 ───────────────────────────────────────
    // 8th Ave Line → A·C·E (blue)
    '22': '#0039A6', '25': '#0039A6', '26': '#0039A6', '79': '#0039A6',
    // Fulton St Line → A·C (blue, Brooklyn)
    '20': '#0039A6', '24': '#0039A6', '27': '#0039A6',
    // Rockaway Line → A (blue, Far Rockaway branch)
    '28': '#0039A6',
    // 6th Ave Line → B·D·F·M (orange)
    '29': '#FF6319', '80': '#FF6319',
    // Queens Blvd Line → E·F·M·R (orange/yellow shared)
    '30': '#FF6319', '42': '#FF6319', '48': '#FF6319', '74': '#FF6319', '75': '#FF6319',
    // Culver/6th Ave Line → F (orange)
    '49': '#FF6319',
    // Crosstown Line → G (light green)
    '52': '#6CBE45', '85': '#6CBE45',
    // Archer Ave Line → J·Z (brown, Jamaica)
    '41': '#996633', '46': '#996633',
    // 63rd St Line → F·Q connector
    '67': '#FF6319', '82': '#FF6319', '84': '#FF6319',
 
    // ── BMT Division 3 ───────────────────────────────────────
    // Broadway Line → N·Q·R·W (yellow)
    '63': '#FCCC0A', '65': '#FCCC0A',
    // 4th Ave Line → N·R (yellow)
    '64': '#FCCC0A', '77': '#FCCC0A', '78': '#FCCC0A',
    // Sea Beach Line → N (yellow)
    '61': '#FCCC0A',
    // West End Line → D (orange, BMT)
    '36': '#FF6319', '40': '#FF6319',
    // Brighton/Culver Line → B·Q (orange/yellow)
    '50': '#FCCC0A',
    // Manhattan Bridge Line → B·D·N·Q
    '62': '#FF6319',
    // Jamaica Line → J·Z (brown)
    '56': '#996633',
    // Nassau St Line → J·Z (brown)
    '57': '#996633',
    // 14th St/Canarsie Line → L (silver)
    '58': '#A7A9AC',
    // Myrtle Ave Line → M (orange)
    '59': '#FF6319',
    // Flushing/Astoria Line → N·W / Astoria branch
    '19': '#FCCC0A',
    // Astoria Line → N·W (yellow)
    '66': '#FCCC0A', '68': '#FCCC0A',
    // Franklin Ave Shuttle → S (gray)
    '35': '#808183',
    // Staten Island Railway (slate)
    '70': '#6D6E71',
 
    // ── Letter routes (if present in other datasets) ──────────
  ...
  function getRouteColor(properties) {
    const route = properties.ROUTE?.trim();
    return ROUTE_COLORS[route] ?? '#555555';
  }
 
  // Load subway lines
  const linesPromise = fetch('/subway_lines.geojson')
    ...
    .then(data => {
      ...
      console.log(`[map.js] Loaded ${lineFeatures.length} subway line segments`);
 
      window.L.geoJSON({ ...data, features: lineFeatures }, {
        style: feature => ({
          color:   getRouteColor(feature.properties),
          weight:  3,
          opacity: 0.85,
        }),
        onEachFeature: (feature, layer) => {
          const p = feature.properties;
          const label = p.SUBWAY_LABEL || p.LINE || `Route ${p.ROUTE}` || 'Unknown';
          layer.bindPopup(`
            <strong>${label}</strong><br>
            ${p.ROUTE   ? `<span>Route: ${p.ROUTE}</span><br>`   : ''}
            ${p.DIVISION ? `<span>Division: ${p.DIVISION}</span>` : ''}
          `);
        },
      }).addTo(_leafletMap);
    })
    .catch(err => console.error('[map.js] Failed to load subway lines:', err));
 
  // Load subway stations
  fetch('/mta_subway_lines.geojson')
    .then(r => {
      if (!r.ok) throw new Error(`HTTP ${r.status}`); // file not found = skip silently
      return r.json();
    })
    .then(data => {
      const stationFeatures = data.features.filter(f =>
        f.geometry?.type === 'Point'
      );
      console.log(`[map.js] Loaded ${stationFeatures.length} subway stations`);
 
      const stationIcon = window.L.divIcon({
        className: '',
        html: `<div style="
          width:7px;height:7px;border-radius:50%;
          background:#1c1c1c;border:2px solid #aaa;
        "></div>`,
        iconAnchor: [4, 4],
      });
      ...
      window.L.geoJSON(data, {
        ...
        pointToLayer: (feature, latlng) =>
          window.L.marker(latlng, { icon: stationIcon }),
        onEachFeature: (feature, layer) => {
          const p = feature.properties;
          // NYC stations GeoJSON uses NAME and LINE fields
          const name  = p.NAME  || p.name  || p.STATION_NAME || 'Station';
          const lines = p.LINE  || p.line  || p.ROUTES       || '';
          layer.bindPopup(`
            <strong>${name}</strong>
            ${lines ? `<br><span style="color:#aaa">${lines}</span>` : ''}
          `);
        },
      }).addTo(_leafletMap);
    })
    .catch(() => {
      // subway_stations.geojson not present — skip silently, lines-only is fine
      console.info('[map.js] No subway_stations.geojson found — station markers skipped.');
    });
}
