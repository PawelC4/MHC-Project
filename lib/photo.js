...
export async function verifyPhoto(imageBase64, mediaType, stationName, questDescription) {
  console.info('[photo.js] verifyPhoto() — using stub. Connect ANTHROPIC_API_KEY via backend to enable real verification.');

  // ── STUB: Simulate verification delay ─────────────────────────────
  await new Promise(r => setTimeout(r, 1800));

  // Stub always succeeds — replace with real API call via backend proxy.
  return {
    success: true,
    message: `Great shot! Your photo from ${stationName} has been verified.`,
    confidence: 0.91,
    tags: ['subway', 'nyc', 'urban'],
  };

  /*
  // ── LIVE implementation (uncomment + build backend route) ─────────
  const response = await fetch('/api/verify-photo', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ imageBase64, mediaType, stationName, questDescription }),
  });
  ...
  return {
    ...
    message: data.reason ?? (data.verified ? 'Photo verified!' : 'Photo could not be verified.'),
