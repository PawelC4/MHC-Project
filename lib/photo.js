...
export async function verifyPhoto(imageBase64, mediaType, stationName, questDescription) {
  console.info('[photo.js] verifyPhoto() — Asking the Gemini AI Judge...');
  ...
  try {
    ...
    // The Gemini route returns exactly what the frontend needs to show the popup
    return {
      success: data.success,
      message: data.message,
      confidence: 0.99, // Gemini gives a definitive yes/no, so we default to high confidence
      tags: ['gemini-verified'], // Placeholder tags so the frontend doesn't break
    };

  } catch (error) {
    console.error('[photo.js] Verification request failed:', error);
    throw new Error('Could not connect to the AI Judge. Please check your internet connection and try again.');
  }
}
